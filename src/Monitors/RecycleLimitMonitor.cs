//------------------------------------------------------------------------------
// RecycleLimitMonitor.cs - based on RecycleLimitMonitor from .Net Framework
//
...
namespace CacheInstrumentation.Monitors
{
    public class RecycleLimitObserver : IObserver<RecycleLimitInfo> {
        ...
        public void OnNext(RecycleLimitInfo recycleLimitInfo) {
            ...
            long trimmedOrExpired = ReflectionUtils.HostEnv_TrimCache(_lastTrimPercent);
    ...
    public class RecycleLimitMonitor : MarshalByRefObject {
            ...
            if (_isStarted) {
                ...
                foreach (IObserver<RecycleLimitInfo> obs in observers) {
                    try {
                        ...
                        ReflectionUtils.ReportUnhandledException(e, new string[] { SR.GetString(SR.Unhandled_Monitor_Exception, "RaiseRecycleLimitEvent", "RecycleLimitMonitor") });
                    }
                }
        ...
        void GetSingleton() {
            if (_defaultDomainSingleton == null && !AppDomain.CurrentDomain.IsDefaultAppDomain()) {
                lock (_singletonLock) {
                    if (_defaultDomainSingleton == null) {
                        AppDomain defaultDomain = AppDomain.CurrentDomain;
                        defaultDomain.SetData(_pbLimit, InstrumentedMemoryMonitor.ProcessPrivateBytesLimit);
        ...
        public class RecycleLimitMonitorSingleton : MarshalByRefObject {
            ...
            const int HIGH_FREQ_INTERVAL_MS = 5000;
            const int MEDIUM_FREQ_INTERVAL_S = 30;
            const int MEDIUM_FREQ_INTERVAL_MS = 30000;
            const int LOW_FREQ_INTERVAL_S = 120;
            const int LOW_FREQ_INTERVAL_MS = 120000;
            ...
            internal RecycleLimitMonitorSingleton(long privateBytesLimit) {
                ...
                _minMaxDelta = 2 * InstrumentedMemoryMonitor.MEGABYTE * Environment.ProcessorCount;
            ...
            private bool AlertProxyMonitors(long current, long limit, RecycleLimitNotificationFrequency frequency) {
                ...
                foreach (KeyValuePair<RecycleLimitMonitor, string> pair in proxies) {
                    try {
                        ...
                        var ac = ReflectionUtils.GetLockableAppDomainContext(_appManager, pair.Value);
                        if (ac != null) {
                            lock (ac) {
