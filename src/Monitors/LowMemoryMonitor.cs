//------------------------------------------------------------------------------
// LowMemoryMonitor.cs - based on LowMemoryMonitor from .Net Framework
//
...
namespace CacheInstrumentation.Monitors
{
    public class LowPhysicalMemoryObserver : IObserver<LowPhysicalMemoryInfo> {
        ...
        public void OnNext(LowPhysicalMemoryInfo lowMemoryInfo) {
            ...
            if (gen2Count != _lastTrimGen2Count) {
                ...
                ReflectionUtils.HostEnv_TrimCache(percent);
    ...
    class LowPhysicalMemoryMonitor {
        ...
        const int MEMORYSTATUS_INTERVAL_5_SECONDS = 5000;
        const int MEMORYSTATUS_INTERVAL_30_SECONDS = 30000;
        ...
        internal LowPhysicalMemoryMonitor() {
            ...
            */
            ...
            long memory = InstrumentedMemoryMonitor.s_totalPhysical;
            ...
            var lkgAppConfig = ReflectionUtils.GetAppLKGConfig();
            try {
                //appConfig = RuntimeConfig.GetAppConfig();
                //cacheConfig = appConfig.Cache;
                var appConfig = ReflectionUtils.GetAppConfig();
                cacheConfig = ReflectionUtils.GetCacheSection(appConfig);
            }
            catch (Exception) {
                //cacheConfig = lkgAppConfig.Cache;
                cacheConfig = ReflectionUtils.GetCacheSection(lkgAppConfig);
            }
        ...
        internal void AdjustTimer(bool disable = false) {
            lock (_timerLock) {
                ...
                if (PressureLast >= PressureHigh) {
                    newPollInterval = Math.Min(newPollInterval, MEMORYSTATUS_INTERVAL_5_SECONDS);
                }

                // When above half the low pressure mark, interval should be 30 seconds or less
                else if (PressureLast > PressureLow / 2) {
                    newPollInterval = Math.Min(newPollInterval, MEMORYSTATUS_INTERVAL_30_SECONDS);
                }
        ...
        internal void MonitorThread(object state) {
            ...
            try {
                ...
                if (PressureLast >= PressureHigh) {
                    ...
                    long beginCount = ReflectionUtils.HttpRuntimeCacheCount();
                    ...
                    long trimmedOrExpired = Math.Max(0, beginCount - ReflectionUtils.HttpRuntimeCacheCount());
                    ...
                    SafeNativeMethods.OutputDebugString("LowMemoryMonitor.MonitorThread:"
                                                    ...
                                                    + ", Milliseconds=" + sw.ElapsedMilliseconds + "\n");
#endif
                    if (!requestGC || ReflectionUtils.ShutdownInProgress(_appManager))
                        return;
        ...
        bool RaiseLowMemoryEvent(int current, int limit) {
            ...
            foreach (IObserver<LowPhysicalMemoryInfo> obs in observers) {
                try {
                    ...
                    ReflectionUtils.ReportUnhandledException(e, new string[] { SR.GetString(SR.Unhandled_Monitor_Exception, "RaiseLowMemoryEvent", "LowMemoryMonitor") });
                }
            }
