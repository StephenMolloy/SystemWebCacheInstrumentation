//------------------------------------------------------------------------------
// CacheUsage.cs - based on CacheUsage from .Net Framework
//
// <copyright file="CacheUsage.cs/cspp" company="Microsoft">
...
namespace CacheInstrumentation.CacheProvider
{
    ...
    internal struct UsageEntryRef
    {
        ...
        {
            if (value is UsageEntryRef ueRef)
            {
                return _ref == ueRef._ref;
            }
                ...
                lock (this)
                {
                    ...
                    if (flushed == 0)
                    {
                        Debug.Trace("CacheUsageFlushTotal", "Flush(" + maxFlush + "," + force + ") removed " + flushed +
                                    " underused items; Time=" + DateTime.Now.ToString());
    ...
    internal class CacheUsage
    {
        ...
        private const byte NUMBUCKETS = (byte)(CacheItemPriority.High);
