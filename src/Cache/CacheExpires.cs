//------------------------------------------------------------------------------
// CacheExpires.cs - based on CacheExpires from .Net Framework
//
// <copyright file="CacheExpires.cs/cspp" company="Microsoft">
...
namespace CacheInstrumentation.CacheProvider
{
    ...
    internal sealed class ExpiresBucket
    {
        ...
        private static readonly TimeSpan COUNT_INTERVAL = new TimeSpan(CacheExpires._tsPerBucket.Ticks / COUNTS_LENGTH);
        ...
        private void RemoveCacheEntryNoLock(CacheEntry cacheEntry)
        {
            ...
            Debug.Trace("CacheExpiresRemove",
                        ...
                        ",expires=" + Utils.ConvertToLocalTime(cacheEntry.UtcExpires));
                ...
                lock (this)
                {
                    ...
                    if (flushed == 0)
                    {
                        Debug.Trace("CacheExpiresFlushTotal", "FlushExpiredItems flushed " + flushed +
                                    " expired items, bucket=" + _bucket + "; Time=" + DateTime.Now.ToString());
    ...
    internal sealed class CacheExpires
    {
        ...
        private static readonly TimeSpan _tsPerCycle = new TimeSpan(NUMBUCKETS * _tsPerBucket.Ticks);
            ...
            if (Interlocked.Exchange(ref _inFlush, 1) == 0)
            {
                try
                {
                    ...
                    if (!checkDelta || utcNow - _utcLastFlush >= MIN_FLUSH_INTERVAL || utcNow < _utcLastFlush)
                    {
                        ...
                        Debug.Trace("CacheExpiresFlushTotal", "FlushExpiredItems flushed a total of " + flushed + " items; Time=" + DateTime.Now.ToString());
#if DBG
                        Debug.Dump("CacheExpiresFlush", this);
#endif
                    }
