//------------------------------------------------------------------------------
// CacheEntry.cs - based on CacheEntry from .Net Framework
//
            ...
            if (IsPublic) {
                try {
                    ...
                    if (HttpContext.Current == null) {
                        ...
                        using (ReflectionUtils.GetAppImpersonationContext()) {
                    ...
                    ReflectionUtils.HttpAppFactory_RaiseError(e);

                    try {
                        //WebBaseEvent.RaiseRuntimeError(e, this);
                        ReflectionUtils.WebEvent_RaiseRuntimeError(e, this);
                    }
                    catch {
                    }
                }
            ...
            if (onRemovedTargets != null) {
                if (targets != null) {
                    foreach (object target in targets) {
                        if (target is CacheDependency dependency) {
                            dependency.ItemRemoved();
                        }
                        else {
                ...
                else if (onRemovedTargets is CacheItemRemovedCallback removedCB) {
                    CallCacheItemRemovedCallback(removedCB, reason);
                }
                else {
        ...
        internal void RemoveDependent(CacheDependency dependency) {
            lock (this) {
                if (_onRemovedTargets != null) {
                    ...
                    else if (_onRemovedTargets is Hashtable h)
                    {
                        h.Remove(dependency);
                        if (h.Count == 0)
                        {
