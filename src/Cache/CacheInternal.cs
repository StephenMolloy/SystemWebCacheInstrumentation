//------------------------------------------------------------------------------
// CacheCommon.cs - based on CacheInternal from .Net Framework
//
...
namespace CacheInstrumentation.CacheProvider
{
    ...
    using Debug = CacheInstrumentation.Debug;
    ...
    abstract class CacheInternal : IDisposable {
...
#endif
                int numSubCaches = 0;
                uint numCPUs = (uint) Environment.ProcessorCount;
        ...
        internal object DoGet(bool isPublic, string key, CacheGetOptions getOptions) {
            ...
            entry = UpdateCache(cacheKey, null, false, CacheItemRemovedReason.Removed, out _);
            if (entry != null) {
        ...
        internal object DoInsert(
            ...
            using (dependencies) {
                ...
                entry = UpdateCache(entry, entry, replace, CacheItemRemovedReason.Removed, out _);
        ...
        internal object DoRemove(CacheKey cacheKey, CacheItemRemovedReason reason)  {
            ...
            UpdateCache(cacheKey, null, true, reason, out object valueOld);
    ...
    sealed class CacheSingle : CacheInternal {
        ...
        internal override long TrimIfNecessary(int percent) {
            ...
            // do we need to trim?
            if (toTrim <= 0 || ReflectionUtils.HostEnv_ShutdownInitated()) {
                return 0;
            }
